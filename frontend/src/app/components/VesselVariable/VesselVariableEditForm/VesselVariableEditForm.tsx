...
const VesselVariableEditForm: React.FC<VesselVariableEditFormProps> = ({ vesselId, variableId, onSuccess }) => {
  const {
    handleSubmit,
    control,
    setValue,
  ...
  const onSubmit = async (data: any) => {
    try {
      await axios.put(
        `${API_BASE_URL}/api/vessel-variable/update-vessel-variable/${variableId}`,
        {
          vessel: {
            connect: {
              id: vesselId,
  ...
  return (
    <Form className="variable-form" onSubmit={handleSubmit(onSubmit)}>
      <Row className="mb-3">
        <Col md={6}>
          <Form.Group>
